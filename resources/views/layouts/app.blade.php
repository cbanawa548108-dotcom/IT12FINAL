    ...
    <aside class="sidebar text-white w-72 flex-shrink-0 hidden md:flex flex-col p-6 shadow-2xl">
        ...
        <nav class="flex-1 flex flex-col space-y-2">
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-content">
                        ...
                        <a href="{{ route('inventory.audit') }}" class="dropdown-item {{ request()->routeIs('inventory.audit') ? 'bg-green-600' : '' }}">
                            <span class="material-icons mr-2" style="font-size: 18px;">history</span> Audit Log
                        </a>
                        <a href="{{ route('inventory.reports') }}" class="dropdown-item {{ request()->routeIs('inventory.reports') ? 'bg-green-600' : '' }}">
                            <span class="material-icons mr-2" style="font-size: 18px;">assessment</span> Inventory Reports
                        </a>
                    </div>
                </div>
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-content">
                    @if(auth()->user()->role === 'admin')
                        <a href="{{ route('archive.suppliers') }}" class="dropdown-item {{ request()->routeIs('archive.suppliers') ? 'bg-green-600' : '' }}">
                            <span class="material-icons mr-2" style="font-size: 18px;">folder_special</span> Supplier Archive
                        </a>
                        <a href="{{ route('archive.customers') }}" class="dropdown-item {{ request()->routeIs('archive.customers') ? 'bg-green-600' : '' }}">
                            <span class="material-icons mr-2" style="font-size: 18px;">folder_shared</span> Customer Archive
                        </a>
            ...
            @if(in_array(auth()->user()->role, ['admin', 'cashier']))
                <a href="{{ route('customers.index') }}" class="nav-link flex items-center p-3 rounded-lg hover:bg-green-600 hover:scale-105 transition shadow-sm {{ request()->routeIs('customers.*') ? 'bg-green-600 active' : '' }}">
                    <span class="material-icons mr-3">people</span>
                    <span class="font-medium">Customers</span>
                </a>

                <a href="{{ route('sales.index') }}" class="nav-link flex items-center p-3 rounded-lg hover:bg-green-600 hover:scale-105 transition shadow-sm {{ request()->routeIs('sales.*') ? 'bg-green-600 active' : '' }}">
                    <span class="material-icons mr-3">sell</span>
                    <span class="font-medium">Sales</span>
                </a>

                <a href="{{ route('sales.report') }}" class="nav-link flex items-center p-3 rounded-lg hover:bg-green-600 hover:scale-105 transition shadow-sm {{ request()->routeIs('sales.report') ? 'bg-green-600 active' : '' }}">
            ...
            @if(auth()->user()->role === 'admin')
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-content">
                        <a href="{{ route('users.list-managers') }}" class="dropdown-item {{ request()->routeIs('users.list-managers') ? 'bg-green-600' : '' }}">
                            <span class="material-icons mr-2" style="font-size: 18px;">people</span> All Managers
                        </a>
                    </div>
                </div>
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-content">
                        <a href="{{ route('users.list-cashiers') }}" class="dropdown-item {{ request()->routeIs('users.list-cashiers') ? 'bg-green-600' : '' }}">
                            <span class="material-icons mr-2" style="font-size: 18px;">people</span> All Cashiers
                        </a>
                    </div>
                </div>
                ...
                <div class="text-xs text-green-200 px-3 py-2 font-bold uppercase tracking-wider flex items-center">
                    <span class="material-icons mr-2" style="font-size: 16px;">security</span>
                    Security
                </div>

                <a href="{{ route('login.audit') }}"
                   class="nav-link flex items-center p-3 rounded-lg hover:bg-green-600 hover:scale-105 transition shadow-sm
                   {{ request()->routeIs('login.audit') ? 'bg-green-600 active' : '' }}">
                    <span class="material-icons mr-3">security</span>
                    <span class="font-medium">Login Audit Logs</span>
                </a>

                <a href="{{ route('blocked-ips.index') }}"
                   class="nav-link flex items-center p-3 rounded-lg hover:bg-green-600 hover:scale-105 transition shadow-sm
                   {{ request()->routeIs('blocked-ips.*') ? 'bg-green-600 active' : '' }}">
                    <span class="material-icons mr-3">block</span>
                    <span class="font-medium">Blocked IPs</span>
                </a>

                <a href="{{ route('password-change-logs.index') }}"
                   class="nav-link flex items-center p-3 rounded-lg hover:bg-green-600 hover:scale-105 transition shadow-sm
                   {{ request()->routeIs('password-change-logs.*') ? 'bg-green-600 active' : '' }}">
                    <span class="material-icons mr-3">lock_reset</span>
                    <span class="font-medium">Password Change Logs</span>
                </a>
            @endif
        </nav>
