    ...
    <div class="bg-white rounded-lg shadow-md p-4">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div class="lg:col-span-2">
                ...
                <input type="text" 
                       id="searchInput"
                       placeholder="Search by product, variety, unit..." 
        ...
        <div class="mt-3 flex justify-end">
            <button id="clearFilters" 
                    class="px-4 py-2 text-sm bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg font-medium transition">
                Clear Filters
            </button>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200 bg-white" id="stocksTableBody">
                ...
                <tr class="hover:bg-green-50 transition stock-row" 
                    ...
                    data-critical="{{ $stock->critical_level }}"
                    data-expiry="{{ $stock->expiry_date ? $stock->expiry_date->format('Y-m-d') : '' }}"
                    data-days-left="{{ $stock->expiry_date ? \Carbon\Carbon::now()->startOfDay()->diffInDays($stock->expiry_date->startOfDay(), false) : '' }}">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function applyFilters() {
        ...
        filteredRows = rows.filter(row => {
            ...
            const critical = parseInt(row.getAttribute('data-critical')) || 0;
            const daysLeft = row.getAttribute('data-days-left');
            
            let matchesStatus = true;
            if (statusValue === 'low') {
                matchesStatus = quantity <= critical;
            } else if (statusValue === 'normal') {
                matchesStatus = quantity > critical;
            } else if (statusValue === 'expired') {
                matchesStatus = daysLeft !== '' && parseInt(daysLeft) <= 0;
            } else if (statusValue === 'expiring') {
                matchesStatus = daysLeft !== '' && parseInt(daysLeft) > 0 && parseInt(daysLeft) <= 7;
            }
    ...
    function updateTable() {
        ...
        if (totalRows > 0) {
            ...
            noResults.querySelector('td').textContent = 'No stock records found matching your filters';
        }
