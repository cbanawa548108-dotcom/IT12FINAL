                ...
                <tbody id="managersTableBody">
                    @forelse($managers as $manager)
                        <tr class="border-b hover:bg-green-50 transition"
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600">{{ $manager->created_at->format('M d, Y') }}</td>
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('users.edit', $manager->User_ID) }}"
                                       class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm flex items-center">
                                        <span class="material-icons text-sm mr-1">edit</span>
                                        Edit
                                    </a>
                                    <button type="button"
                                            onclick="openArchiveModal('{{ $manager->User_ID }}', '{{ $manager->fname }} {{ $manager->lname }}')"
            ...
            <div class="flex items-start">
                ...
                <div>
                    ...
                    <div class="mt-2 flex items-center gap-2">
                        <span class="px-2 py-0.5 text-xs font-bold rounded-full bg-green-100 text-green-800">Manager</span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const rows = document.querySelectorAll('#managersTableBody tr:not(#noResults)');
    ...
    searchInput.addEventListener('input', function() {
        ...
        if (noResults && rows.length > 0) {
            ...
            if (count === 0 && term) {
                noResults.querySelector('td').innerHTML = `<div class="flex flex-col items-center"><p class="text-lg">No managers found matching "${term}"</p></div>`;
            }
        }
    });
});
